...
router.put("/:id", async (req,res) => {
    if(req.body.userId === req.params.id || req.body.isAdmin){
        ...
        return res.status(403).json("you can only update account")
    }
})
...
router.delete("/:id", async (req,res) => {
    if(req.body.userId === req.params.id || req.body.isAdmin){
        ...
        return res.status(403).json("you can only delete account")
    }
})
...
router.get("/suggested/:userId",  async (req,res)=>{
    try {
        ...
        const alluser = await User.find( { "_id": { "$nin": friendList} })
...
router.put("/:id/unfollow", async (req,res)=>{
    if(req.body.userId !== req.params.id) {
        ...
        if (user.followers.includes(req.body.userId)){
            ...
            res.status(200).json("user has been unfolllowed")
